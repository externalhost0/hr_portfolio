window.namespace = function (code) {
	code();
};
...
namespace(function () {
	...
	var tracks = {
		start: new Audio(relative + "sounds/panel_start_tracing.aac"),
		success: new Audio(relative + "sounds/panel_success.aac"),
		fail: new Audio(relative + "sounds/panel_failure.aac"),
		abort: new Audio(relative + "sounds/panel_abort_tracing.aac"),
	};
	...
	window.loadHeader = function (titleText) {
		...
		sourceLink.onpointerdown = function () {
			window.open("https://github.com/jbzdarkid/jbzdarkid.github.io", "_blank");
		};
		...
		sens.style.backgroundImage =
			"linear-gradient(to right, " + window.ALT_BACKGROUND + ", " + window.ACTIVE_COLOR + ")";
		...
		wittleLabel.onpointerdown = function () {
			wittleTracing.onpointerdown();
		};
  ...
  */
	...
	window.showSolution = function (puzzle, paths, num, suffix) {
		...
		if (paths[num] != null) {
			if (puzzle instanceof Array) {
				// Special case for multiple related panels
				...
				// Default case for a single panel
				// Save the current path on the puzzle object (so that we can pass it along with publishing)
	...
	window.addSolveButtons = function () {
		...
		solveManual.onpointerdown = function () {
			solveMode.onpointerdown();
		};
	...
	window.addEventListener("error", function (event) {
		...
		console.error(
			"Please disregard the following CORS exception. It is expected and the request will succeed regardless.",
		);
		var formUrl =
			"https://docs.google.com/forms/d/e/1FAIpQLSeXwtuKTbhXlQ5dUYtGjMQtkseFMBFka0jbBeOwd8tKiJb_ug/formResponse";
		window.fireAndForget(
			"POST",
			formUrl,
			"entry.603156546=" +
				encodeURIComponent(sessionId) +
				"&entry.909077667=" +
				encodeURIComponent(event.error) +
				"&entry.2145672989=" +
				encodeURIComponent(event.filename + ":" + event.lineno) +
				"&entry.587895622=" +
	...
	function sendFeedback(feedback) {
		...
		var formUrl =
			"https://docs.google.com/forms/d/e/1FAIpQLSe6kWD2rC7qaBVExJBUmAhU5qnhLzaY98ZQ3xp6Gq5fkizNHQ/formResponse";
		window.fireAndForget(
			"POST",
			formUrl,
			"entry.1993425622=" +
